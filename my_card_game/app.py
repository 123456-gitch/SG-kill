...
def broadcast_state():
    ...
    for human_player in game.players:
        if human_player.get('is_bot', False):
            continue
...
def run_bot_turn(bot_idx):
    time.sleep(1.2)
    if not game.active or game.current_idx != bot_idx or game.pending_action:
        return
    p = game.players[bot_idx]
...
def run_bot_defense(bot_idx):
    time.sleep(1.2)
    if not game.active or not game.pending_action:
        return
    if game.pending_action['target_idx'] != bot_idx:
        return
...
def execute_play_card(src_idx, card_to_spend, card_to_execute, tgt_idx):
    ...
    if game.actions_left <= 0:
        return False
    if card_to_spend not in src['hand']:
        return False
...
def on_respond_action(data):
    if not game.active or not game.pending_action:
        return
    p = get_player_by_sid(request.sid)
    if not p or game.pending_action['target_idx'] != p['idx']:
        return
...
def on_beishui_decision(data):
    ...
    if not p or game.current_idx != p['idx']:
        return
    if p['status'] != "背水一战" or p['beishui_decided']:
        return
