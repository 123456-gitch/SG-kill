...
def broadcast_state():
    ...
    for human_player in game.players:
        ...
        for p in game.players:
            visible_faction = "隐藏"
            # 只有当本人、或阵营已公开、或已死亡，才会向当前人类玩家公开阵营真实标签
            if p['idx'] == human_idx:
...
def start_turn(idx):
    ...
    if p['status'] == "饮鸩止渴":
        ...
        force_hp_limit(p)
        add_log(f"🧪 【{p['name']}】饮鸩止渴慢毒发作：上限-3 → 当前生命与最大上限为 {p['hp']}/{p['max_hp']}")
    ...
    for p in game.players:
        if p['alive'] and p['status_cooldown'] > 0:
            ...
            if p['status_cooldown'] == 0:
                add_log(f"✨ 【{p['name']}】的【{p['status']}】冷却结束，现在可以在自己回合更换了！")
...
def get_bot_playable_cards(bot_idx):
    ...
    for card in p['hand']:
        if card == "防":
            if p['status'] == "暗度陈仓":
                playable.append(("防", "攻")) # spend 防, execute 攻
        elif card == "长城":
            pass # 长城不可作为主动卡打出
        elif card == "回血":
            if p['hp'] < p['max_hp']:
                playable.append(("回血", "回血"))
        else:
...
def get_bot_target(bot_idx):
    ...
    # Rule 4: 优先识别身份敌友。若有已亮身份的敌对阵营，精准锁定打击
    revealed_enemies = []
    for p in others:
        is_enemy = False
        if bot['faction'] == "司":
            is_enemy = p['faction'] in ["冀", "丁"]
        else: # 冀 或 丁
            is_enemy = p['faction'] == "司"
            
        if is_enemy and p.get('faction_revealed', False):
            revealed_enemies.append(p)
            
    if revealed_enemies:
        return random.choice(revealed_enemies)['idx']
        
    # 如果全场身份尚未明朗，或者敌对阵营暂未浮现/亮明身份，则盲打：随机打另外两个玩家中的一个
...
def run_bot_turn(bot_idx):
    ...
    while game.active and game.current_idx == bot_idx and game.actions_left > 0 and not game.pending_action:
        ...
        if not playable:
            break # 没主动牌了，跳出循环自动结束（Rule 1）

        # Rule 2：有攻先出攻（包含暗度陈仓状态将防转成攻），没攻出别的
        ...
        if attacks:
            spend, execute = random.choice(attacks)
        elif other_attacks:
            spend, execute = random.choice(other_attacks)
        else:
            spend, execute = random.choice(others)

        # 决定目标
        if execute in ["回血", "卡牌大师"]:
            tgt_idx = -1
        else:
...
def run_bot_defense(bot_idx):
    ...
    p = game.players[bot_idx]
    card_name = game.pending_action['card'] # 遭到突袭的目标牌

    # Rule 3：防守时能打防打防，不能打就打长城，实在不行就放弃
    if card_name in ["攻", "荆轲刺秦"]:
        if "防" in p['hand']:
            execute_defense_response(bot_idx, '防')
        elif p['status'] == "暗度陈仓" and "攻" in p['hand']:
            execute_defense_response(bot_idx, '攻_as_防')
        elif "长城" in p['hand']:
            execute_defense_response(bot_idx, '长城')
        else:
            execute_defense_response(bot_idx, '放弃')
    else:
        # 法术牌（顺手、一字马等）不能用【防】挡，人机直接用【长城】完美格挡，若没有则放弃
        if "长城" in p['hand']:
            execute_defense_response(bot_idx, '长城')
        else:
            execute_defense_response(bot_idx, '放弃')
...
def execute_defense_response(idx, resp_type):
    ...
    # 如果攻击效果仍在挂起中（例如对机器人打出荆轲，其闪避了1张防，还需要再挡1张）
    if game.active and game.pending_action and game.pending_action['target_idx'] == idx:
        ...
        # 防御完成，移交控制，若当前回合依然是人机掌控，立刻唤醒其AI决策
        if game.active and not game.pending_action:
            trigger_bot_if_needed()

# ==========================================
# ⚔️ Spend-Execute 核心出牌引擎
# ==========================================
def execute_play_card(src_idx, card_to_spend, card_to_execute, tgt_idx):
    ...
    # 卧薪尝胆自损反噬：每回合只要主动出牌，扣1血，回血牌除外，保底1血不致死
...
def damage_player(idx, amount, reason=""):
    ...
    # ==========================================
    # 🌟 护卫【丁】复活与叛逆者【司】大觉醒补充10张手牌判定
    # ==========================================
    if p['hp'] <= 0 and p['faction'] == "丁" and not p.get('has_revived', False):
        p['has_revived'] = True
        p['hp'] = 2
        p['max_hp'] = max(p['max_hp'], 2)
        p['faction_revealed'] = True # 全场可明示其真实标签
        add_log(f"🔥✨ 守护神【丁】血限归零！原地浴火复活！生命值回到 2 点并补充 2 张牌，全场身份自此大白于天下！")
        draw_cards(idx, 2)
    ...
    if p['hp'] <= 0:
        p['hp'] = 0
        p['alive'] = False
        p['faction_revealed'] = True # 阵亡后向所有人公开
...
def on_respond_action(data):
    ...
    card_name = game.pending_action['card'] # 被攻击的源牌名称
    src_idx = game.pending_action['source_idx']
    tgt_idx = game.pending_action['target_idx']
    
    # 完美防御：任意指向手牌均可用1张【长城】完美隔挡
    if resp_type == '长城' and "长城" in p['hand']:
        p['hand'].remove("长城")
        add_log(f"🧱 【{p['name']}】祭出高耸【长城壁】！完美格挡了本次针对其发动的【{card_name}】效果！")
        game.pending_action = None
        
    # 普通闪避防守：【防】只能响应【攻】或【荆轲刺秦】
    elif resp_type == '防' and "防" in p['hand'] and card_name in ["攻", "荆轲刺秦"]:
        p['hand'].remove("防")
        game.pending_action['required_defenses'] -= 1
    ...
    # 响应结束后，如果已经解决挂载且仍处于游戏态，唤醒人机继续战斗
    if game.active and not game.pending_action:
        trigger_bot_if_needed()
